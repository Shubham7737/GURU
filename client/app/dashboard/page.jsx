          ...
          <div className="courses-grid-view">
            {courses.map((course) =>                <div key={course.id} className="course-card-modern" onClick={() => {
                  ...
                  <div className="card-info">
                    ...
                    <button className="btn btn-primary btn-sm btn-block" onClick={(e) => { e.stopPropagation(); router.push(`/learn/${course.id}`); }}>
                      Continue Learning
                    </button>
                  </div>
                </div>
        ...
        return (
          <div className="live-classes-view">
            <div className="live-hero-card">
              ...
              <button className="btn btn-primary" onClick={() => router.push('/live-session/1')}>Join Session Now</button>
            </div>
          </div>
        );
            ...
            {courses.slice(0, 2).map((course, i) => (
              <div key={i} className="course-card-modern">
                ...
                <div className="card-info">
                  ...
                  <button className="btn btn-primary btn-sm btn-block" style={{ marginTop: '1rem' }} onClick={() => router.push(`/learn/${course.id}`)}>Watch Now</button>
                </div>
              </div>
            ))}
            {courses.length === 0 && (
              ...
              <div className="sessions-grid">
                {sessions.map((sess) => (
                  <SessionCard
                    ...
                    onClick={() => {
                      if (!sess.isLocked) router.push(`/learn/${selectedCourse.id}/session/${sess.id}`);
                    }}
                  />
                ))}
                ...
                <div className="course-list">
                  {courses.slice(0, 2).map((course, i) => (
                    <div key={course.id} className="mini-course-card">
                      ...
                      <button className="play-btn" onClick={() => router.push(`/learn/${course.id}`)}><PlayCircle size={24} /></button>
                    </div>
                  ))}
                  {courses.length === 0 && (
